...
const orderSchema = new mongoose.Schema({
    ...
    order_products: [{ productId: { type: mongoose.Schema.Types.ObjectId, ref: "Product", required: true }, quantity: { type: Number, required: true, min: 1 } }], // order_products: Mảng sản phẩm với productId và số lượng (quantity)
    ...
    order_status_history: [{ status: { type: String, enum: ["pending", "confirm", "shipped", "delivered", "cancelled"] }, timestamp: { type: Date, default: Date.now } }], // order_status_history: Lịch sử thay đổi trạng thái cùng thời gian thay đổi
